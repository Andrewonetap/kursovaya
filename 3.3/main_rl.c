...
#include "app_data_rl.h"       // Содержит определения PADDING, BUTTON_HEIGHT, TOP_NAV_BAR_HEIGHT и т.д.
#include "sqlite_utils_rl.h"
#include "calculations_rl.h"
#include "level1_analysis_rl.h"
#include "level4_analysis_rl.h"
#include "color_themes.h"      // <--- ВКЛЮЧАЕМ НОВЫЙ ФАЙЛ С ТЕМАМИ
...
// Константы для отрисовки таблицы (если они специфичны только для main_rl.c и не нужны глобально)
// Если они уже есть в app_data_rl.h, то здесь их дублировать не нужно.
// #define TABLE_HEADER_HEIGHT 25.0f // Уже в app_data_rl.h
// #define TABLE_ROW_HEIGHT 22.0f    // Уже в app_data_rl.h
// #define TABLE_EPOCH_COL_WIDTH 70.0f // Уже в app_data_rl.h
// #define TABLE_DATA_COL_WIDTH 90.0f  // Уже в app_data_rl.h
#define TABLE_CELL_TEXT_PADDING_X 5.0f // Можно оставить, если специфично для этой таблицы

// Глобальные переменные HIDE_... определены в level1_analysis_rl.c
// и объявлены extern в app_data_rl.h, поэтому здесь их не определяем.
...
static void ApplyGuiThemeStyles(AppData_Raylib *data) {
    ...
    GuiSetStyle(SLIDER, TEXT_COLOR_NORMAL, theme->scrollbar_thumb_bg); // RayGUI ожидает это, хоть и не всегда используется для thumb
    ...
    // Для GuiWindowBox (заголовок окна диалога)
    // RayGUI использует DEFAULT свойства для фона и текста заголовка окна,
    // а BORDER_COLOR_NORMAL для рамки.
    // Если нужен отдельный цвет для заголовка, можно установить:
    // GuiSetStyle(WINDOW_DIALOG, BACKGROUND_COLOR, theme->header_toolbar_bg);
    // GuiSetStyle(WINDOW_DIALOG, TITLE_TEXT_COLOR, theme->text_on_button_active);
    // По умолчанию заголовок будет использовать DEFAULT, BACKGROUND_COLOR (т.е. theme->panel_background)
    // и DEFAULT, TEXT_COLOR_NORMAL (т.е. theme->text_primary)
}
...
static void InitApp(AppData_Raylib *data) {
    ...
    data->screen_width = 1280; // Исходные значения, потом могут быть переопределены
    ...
    data->screen_width = 1176; // Твои актуальные размеры
    data->screen_height = 664;

    // Инициализация и установка начальной темы
    InitializeAllApplicationThemes(); // Вызов функции из color_themes.c
    data->current_theme_id = LIGHT_THEME_ID; // Используем define из color_themes.h
    data->current_theme_colors = app_themes[data->current_theme_id]; // Используем extern массив из color_themes.c

    InitWindow(data->screen_width, data->screen_height, "Анализ данных временных рядов (Raylib)");
    SetTargetFPS(60);

    GuiLoadStyleDefault();    // Сначала загружаем стандартный стиль RayGUI
    ApplyGuiThemeStyles(data); // Затем применяем нашу кастомную тему

    // --- ЗАГРУЗКА ШРИФТА (остается как было) ---
...
void DrawDataTable_RL(AppData_Raylib *data, TableData *tableData, const char* title,
    ...
    // Используем константы из app_data_rl.h
...
static void DrawEditValuesDialog(AppData_Raylib *data) {
    ...
    if (pressed_window_close_button_val) { close_dialog_req = true; }
