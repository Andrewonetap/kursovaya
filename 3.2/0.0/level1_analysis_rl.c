...
static void ResetLevel1Data(AppData_Raylib *data);
...
static void DrawMultiSeriesLineGraph_L1(
    AppData_Raylib *data,
...
static void DrawSimpleLineSeries(Rectangle bounds, double *y_values, int num_points, Color color,
    ...
    // Простая отрисовка без осей и сложного масштабирования
    // Масштабируем Y, чтобы он помещался в bounds.height
    // X масштабируем, чтобы все точки поместились в bounds.width
...
static bool EnsureLevel1PlusECalculations(AppData_Raylib *data) {
    ...
    data->alpha_prime_data = compute_alpha_rl(table_x_plus_e.table, table_x_plus_e.row_count, table_x_plus_e.column_count, data->mu_prime_data);
    if (!data->alpha_prime_data) {
...
static bool EnsureLevel1MinusECalculations(AppData_Raylib *data) {
    ...
    data->alpha_double_prime_data = compute_alpha_rl(table_x_minus_e.table, table_x_minus_e.row_count, table_x_minus_e.column_count, data->mu_double_prime_data);
    if (!data->alpha_double_prime_data) {
