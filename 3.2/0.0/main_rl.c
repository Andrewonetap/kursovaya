...
void DrawDataTable_RL(AppData_Raylib *data, TableData *tableData, const char* title, 
    ...
    for (int r = 0; r < tableData->row_count; r++) {
        ...
        if (!hide_first_col) {
            ...
            if (tableData->epochs) { // Убедимся, что массив эпох существует
                char text_to_draw_in_first_col[32]; // Буфер для текста

                // Определяем, является ли текущая таблица "Итоговой таблицей погрешности"
                // Лучше всего передать это как флаг в DrawDataTable_RL, но пока используем title.
                bool is_conclusion_table_type = (title && strcmp(title, "Итоговая таблица погрешности") == 0);

                if (is_conclusion_table_type) {
                    if (tableData->epochs[r] == -1) { // Ваше специальное значение для "прогноза"
                        strcpy(text_to_draw_in_first_col, "прогноз");
                    } else {
                        sprintf(text_to_draw_in_first_col, "t%d", tableData->epochs[r]);
                    }
                } else {
                    // Стандартное отображение для других таблиц
        ...
        if (tableData->table && tableData->table[r]) {
            for (int c = 0; c < tableData->column_count; c++) { // c - это индекс колонки ДАННЫХ
                const char *current_data_column_name = NULL; // Имя текущей колонки данных

                    if (isValuesTableType) {
                        // Для таблицы "Значения", c напрямую соответствует индексу в column_names
                        if (c < tableData->num_allocated_column_names && tableData->column_names[c]) {
                            current_data_column_name = tableData->column_names[c];
                        }
                    } else {
                        // Для других таблиц (Основная, Сглаживания), имена колонок данных в column_names
                        // смещены на 1 (так как column_names[0] - это Эпоха/M)
                ...
                // Проверка на скрытие колонки "Схема" (только для таблицы "Значения")
                if (isValuesTableType && current_data_column_name && 
                    strcmp(current_data_column_name, data->schema_blob_column_name) == 0 && 
                    HIDE_SCHEMA_COLUMN_IN_VALUES_TABLE) {
                    // current_col_offset_x НЕ увеличивается, так как колонка пропускается (если она не должна занимать место)
                    // Однако, если HIDE_SCHEMA_COLUMN_IN_VALUES_TABLE означает, что она просто не рисуется,
                    // но место под нее в разметке заголовков было выделено, то current_col_offset_x нужно увеличивать.
                    // Судя по вашему коду заголовков, если колонка скрыта, она не получает ширину.
                    // Значит, здесь continue достаточно, и current_col_offset_x для нее не добавлялся.
                    continue; 
                }
                ...
                // Если это колонка BLOB "Схема" в таблице "Значения" (и она не скрыта предыдущим if)
                if (isValuesTableType && current_data_column_name && strcmp(current_data_column_name, data->schema_blob_column_name) == 0) { 
                    ...
                    // --- НАЧАЛО ИЗМЕНЕНИЙ ДЛЯ ФОРМАТИРОВАНИЯ DOUBLE ---
                    const char* format_string_for_double; // Новая переменная для форматной строки
                    ...
                    if (is_conclusion_table_type_for_data) {
                        // c - это индекс колонки ДАННЫХ в tableData->table[r][c]
                        // Для "Итоговой таблицы погрешности", колонки данных это:
                        // c=0: Mz+E
                        // c=1: Mz
                        // c=2: Mz-E
                        // c=3: R
                        // c=4: L
                        if (c == 0 || c == 1 || c == 2) { // Mz+E, Mz, Mz-E
                            format_string_for_double = "%.7f"; // 7 знаков после запятой
                        } else if (c == 3) { // R
                            format_string_for_double = "%.5f"; // 5 знаков (для 0,00196)
                        } else if (c == 4) { // L
                            double abs_val_l = fabs(tableData->table[r][c]);
                            if (abs_val_l > 0 && (abs_val_l < 0.0001 || abs_val_l >= 100000.0)) { // Экспоненциальная для очень малых или больших
                                format_string_for_double = "%.2E";
                            } else {
                                format_string_for_double = "%.6f"; // 6 знаков для обычных L
                            }
                        } else {
                            format_string_for_double = "%.4f"; // Запасной вариант, если колонок больше
                        }
                    } else {
                        // Ваша существующая логика форматирования для других таблиц (например, для Alpha)
                        bool treat_as_potentially_small_original = false;
                        if (title) { // Проверка по заголовку таблицы
                            if (strstr(title, "Alpha") != NULL || strstr(title, "alpha") != NULL ||
                                strstr(title, "Альфа") != NULL || strstr(title, "альфа") != NULL ||
                                strstr(title, "Сглаживание α") != NULL ||
                                strstr(title, "Сглаживание Alpha") != NULL ) {
                                treat_as_potentially_small_original = true;
                            }
                        }
                        if (!treat_as_potentially_small_original && current_data_column_name) { // Проверка по имени колонки
                            if (strstr(current_data_column_name, "Alpha") != NULL ||
                                strstr(current_data_column_name, "alpha") != NULL ||
                                strstr(current_data_column_name, "Альфа") != NULL ||
                                strstr(current_data_column_name, "альфа") != NULL) {
                                treat_as_potentially_small_original = true;
                            }
                        }
                        ...
                        if (treat_as_potentially_small_original) {
                            ...
                            if (abs_val > 0 && (abs_val < 0.001 || abs_val >= 100000.0)) {
                                format_string_for_double = "%.3E";
                            } else if (abs_val > 0 && abs_val < 1.0) {
                                format_string_for_double = "%.5f";
                            } else {
                                format_string_for_double = "%.4f";
                            }
                        } else {
                            format_string_for_double = "%.4f"; // Стандартный формат для других таблиц
                        }
                    }
...
static void DrawTopNavigationBar(AppData_Raylib *data, Rectangle bounds) {
    ...
    if (GuiButton((Rectangle){current_x, button_y, open_button_width, BUTTON_HEIGHT}, open_text_str)) {
        ...
        if (selectedFilePath) {
            ...
            // >>> ДОБАВИТЬ СБРОС СОСТОЯНИЯ ГРАФИКОВ ЗДЕСЬ <<<
            for (int i = 0; i < GRAPH_ID_COUNT; i++) {
                data->graph_states[i].is_active = false; // Помечаем, что масштаб/смещение нужно пересчитать
                data->graph_states[i].scale = (Vector2){0.0f, 0.0f}; // Также сбрасываем scale, чтобы условие точно сработало
                data->graph_states[i].offset = (Vector2){0.0f, 0.0f};
            }
...
static void DrawEditMainTableRowDialog(AppData_Raylib *data) {
    if (!data || !data->show_main_edit_dialog || data->main_edit_dialog_row_index < 0 || 
        ...
        if (data && data->show_main_edit_dialog) { // Добавим проверку, чтобы не сбрасывать без надобности
            data->show_main_edit_dialog = false; 
        }
        return;
    }
    ...
    if (pressed_window_close_button) {
