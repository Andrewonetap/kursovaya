...
double *calculate_new_row_rl(const TableData *table_data) {
    ...
    // srand(time(NULL)); // Лучше вызывать srand один раз при запуске программы, а не здесь.
                        // Если оставить здесь, то при быстром многократном вызове rand() будет давать те же последовательности.
