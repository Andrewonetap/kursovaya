...
// считает альфу
double *compute_alpha_rl(double **table_data, int row_count, int column_count, double *mu_values, const char *current_main_table_name) {
    if (!table_data || !mu_values || row_count <= 0 || !current_main_table_name) { // Проверка current_main_table_name
        debug_print_calc_rl("compute_alpha_rl: Входные данные невалидны (нет таблицы, μ, строк <= 0 или имени главной таблицы).\n");
        return NULL;
    }
    ...
    debug_print_calc_rl("compute_alpha_rl: Начало вычисления α для %d строк, %d колонок. Главная таблица: '%s'.\n", row_count, column_count, current_main_table_name);

    bool apply_rounding_to_zero = (strcmp(current_main_table_name, "Данные") != 0);
    if (apply_rounding_to_zero) {
        debug_print_calc_rl("compute_alpha_rl: Обнуление Alpha будет применено, так как таблица не 'Данные'.\n");
    }

    if (column_count == 0) {
        debug_print_calc_rl("compute_alpha_rl: Количество колонок данных равно 0. Все α будут 0.\n");
        // calloc уже заполнил нулями, так что здесь ничего делать не нужно
        return alpha_values;
    }

    if (table_data[0] == NULL) {
        debug_print_calc_rl("compute_alpha_rl: Ошибка, table_data[0] == NULL. Невозможно вычислить α.\n");
        // calloc уже заполнил нулями
        return alpha_values;
    }
    ...
    for (int i = 1; i < row_count; i++) { // по строкам начиная со второй
        ...
        // Если нужно обнулять, делаем это сразу и пропускаем вычисления
        if (apply_rounding_to_zero) {
            alpha_values[i] = 0.0;
            continue; // Переходим к следующей итерации цикла
        }

        // Если не обнуляем, производим стандартные вычисления
        ...
        if (fabs(denominator) < 1e-9) { // Используем fabs для double
            alpha_values[i] = 0.0;
        } else {
